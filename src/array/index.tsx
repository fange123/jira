...
export const TsReactTest = () => {
  const persons: { name: string; age: number; id: number }[] = [
    { name: "jack", age: 25, id: Math.random() },
    { name: "ma", age: 22, id: Math.random() },
  ];
  ...
  return (
    <div>
      {/*期待: 点击以后增加 john */}
      <button onClick={() => add({ name: "john", age: 22, id: 1 })}>
        add john
      </button>
      {/*期待: 点击以后删除第一项*/}
      ...
      {value.map((person, index) => (
        <div style={{ marginBottom: "30px" }} key={`${Math.random()}`}>
